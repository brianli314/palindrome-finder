...
def gap_sort(bin_size):
    ...
    x_ticks = list(range(0, len(output.keys()), max(1, max(output.keys())//(2*bin_size))))
...
def gap_sort_length(bin_size):
    ...
    x_labels = [f"{i*bin_size} - {(i+1)*bin_size}" for i in x_ticks]
    plt.xticks(x_ticks, x_labels)
    plt.title("Gap length to average arm length")
    plt.xlabel("Gap length")
    plt.ylabel("Average arm length")
    plt.show()
